...
export function activate(context: vscode.ExtensionContext): void {
	...
	let sectionVisibility = normalizeSectionVisibility(
		context.globalState.get(SECTION_VISIBILITY_STORAGE_KEY),
	);
	let sectionCollapseState = normalizeSectionCollapseState(
		context.globalState.get(SECTION_COLLAPSE_STORAGE_KEY),
	);
	...
	const actions: ProjectSwitcherActions = {
		saveCurrentProject: async () => {
			...
			if (!currentInput) {
				await vscode.window.showWarningMessage(
					'No active workspace folder or workspace file to save.',
				);
				return;
			}
		...
		addWorkspaceProject: async () => {
			...
			const defaultName =
				path.basename(workspaceUri.fsPath, '.code-workspace') || 'Workspace';
			...
			await vscode.window.withProgress(
				...
				async () => {
					...
					void vscode.window.showInformationMessage(
						`Cloned and added "${projectName}" to projects.`,
					);
				},
			);
		},
	...
	context.subscriptions.push(
		...
		vscode.window.registerWebviewViewProvider(
			ProjectSwitcherViewProvider.viewType,
			viewProvider,
			{
				webviewOptions: {
	...
	register(COMMANDS.openProjectInNewWindow, async (projectContext?: unknown) => {
		const id =
			resolveProjectId(projectContext) ??
			(await pickProjectId(store, 'Open project in new window'));
		if (!id) {
			return;
		}
	...
	register(COMMANDS.renameProject, async (projectContext?: unknown) => {
		const id =
			resolveProjectId(projectContext) ?? (await pickProjectId(store, 'Rename project'));
		if (!id) {
			return;
		}
	...
	register(COMMANDS.removeProject, async (projectContext?: unknown) => {
		const id =
			resolveProjectId(projectContext) ?? (await pickProjectId(store, 'Remove project'));
		if (!id) {
			return;
		}
	...
	register(COMMANDS.removeGroup, async (groupContext?: unknown) => {
		const id =
			resolveProjectGroupId(groupContext) ??
			(await pickGroupId(store, 'Remove project group'));
		if (!id) {
			return;
		}
	...
	register(COMMANDS.togglePinProject, async (projectContext?: unknown) => {
		const id =
			resolveProjectId(projectContext) ??
			(await pickProjectId(store, 'Pin or unpin project'));
		if (!id) {
			return;
		}
	...
	register(COMMANDS.unpinProject, async (projectContext?: unknown) => {
		const id =
			resolveProjectId(projectContext) ?? (await pickProjectId(store, 'Unpin project'));
		if (!id) {
			return;
		}
	...
	register(COMMANDS.revealInFinder, async (projectContext?: unknown) => {
		const id =
			resolveProjectId(projectContext) ??
			(await pickProjectId(store, 'Reveal project in Finder'));
		if (!id) {
			return;
		}
...
interface QuickPeekItem {
	...
	description: string;
	detail: string;
	project: { id: string; uri: string; isVirtual: boolean };
}

function buildQuickPeekItem(
	project: { id: string; name: string; uri: string },
	detailPrefix: string,
): QuickPeekItem {
	const fullPath = getFilePath(project.uri);
	const shortName = getShortName(project.uri);
	const branch = fullPath ? readGitBranchSync(fullPath) : null;
	const displayPath = compactPath(project.uri);
	const isVirtual = project.id.startsWith('group:');
	...
	return {
		...
		detail: '',
		project: { id: project.id, uri: project.uri, isVirtual },
	};
}

async function showQuickPeek(store: ProjectStore, actions: ProjectSwitcherActions): Promise<void> {
	...
	const groupSections = resolveProjectGroupSections({
		...
		listGroupChildren: (group) => {
			...
			try {
				const entries: import('fs').Dirent[] = require('fs').readdirSync(rootPath, {
					withFileTypes: true,
				});
				return entries
					.filter((entry: import('fs').Dirent) => entry.isDirectory())
					.map((entry: import('fs').Dirent) => {
						const folderPath = require('path').join(rootPath, entry.name);
						return {
							name: entry.name,
							uri: require('vscode').Uri.file(folderPath).toString(),
						};
					});
			} catch {
				return [];
	...
	const items: QuickPeekItem[] = [
		...savedProjects.map((project) => buildQuickPeekItem(project, project.name)),
		...groupSections.flatMap((section) =>
			section.projects.map((project) => buildQuickPeekItem(project, section.title)),
		),
	];

	if (items.length === 0) {
		await vscode.window.showInformationMessage('No projects saved yet.');
		return;
	}
	...
	if (selected.project.isVirtual) {
		await actions.openProjectUri(selected.project.uri);
	} else {
		await actions.openProject(selected.project.id);
	}
}

function getShortName(uriString: string): string {
	try {
...
function applySectionVisibilityContext(sectionVisibility: SectionVisibility): void {
	void vscode.commands.executeCommand(
		'setContext',
		SECTION_VISIBILITY_CONTEXT.current,
		sectionVisibility.current,
	);
	void vscode.commands.executeCommand(
		'setContext',
		SECTION_VISIBILITY_CONTEXT.recent,
		sectionVisibility.recent,
	);
	void vscode.commands.executeCommand(
		'setContext',
		SECTION_VISIBILITY_CONTEXT.pinned,
		sectionVisibility.pinned,
	);
	void vscode.commands.executeCommand(
		'setContext',
		SECTION_VISIBILITY_CONTEXT.projects,
		sectionVisibility.projects,
	);
	void vscode.commands.executeCommand(
		'setContext',
		SECTION_VISIBILITY_CONTEXT.groups,
